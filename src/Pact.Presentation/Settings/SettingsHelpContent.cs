...
public static class SettingsHelpContent
{
	...
	private const string LaunchProfilesBody = """
        ...
        Command is the command that starts a brand-new session for this profile. Resume command is copied into a session as its initial resume command when that session is created, but it is not consulted again after that. A session's own resume command restores that exact conversation when it names one (for example "codex resume <id>" or "claude --resume <id>"), and opens the agent's conversation picker when it names none ("codex resume", "claude --resume") — which is how a session is recovered after Pact exits unexpectedly. Any other shape is not a usable resume invocation, so restoring falls back to the session's own launch command instead.
